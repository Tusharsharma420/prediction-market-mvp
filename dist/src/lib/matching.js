...
exports.enqueueOrderProcessing = enqueueOrderProcessing;
exports.broadcastMarketUpdates = broadcastMarketUpdates;
const db_1 = __importDefault(require("./db"));
const marketQueues = new Map();
function enqueueOrderProcessing(marketId, orderId, io) {
...
async function matchOrders(marketId, orderId, io) {
    // 1. Fetch the incoming order
    const newOrder = await db_1.default.order.findUnique({
        where: { id: orderId },
        include: { user: true }
    });
    ...
    // 2. Fetch the market
    const market = await db_1.default.market.findUnique({
        where: { id: marketId }
    });
    ...
    const oppositeOrders = await db_1.default.order.findMany({
        where: {
            marketId,
...
async function broadcastMarketUpdates(marketId, io) {
    if (!io)
        return;
    ...
    const formatBook = (book, sortDesc) => {
    ...
    // Fetch recent trades
    const recentTrades = await db_1.default.trade.findMany({
        where: { marketId },
